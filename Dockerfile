...
# - Within the image, the application files (source files) exist under the /app directory.
#WORKDIR /app
#COPY /toDoAppWithLogin .
...
ENTRYPOINT ["java","-cp","target/toDoAppWithLogin.jar","org.springframework.boot.loader.JarLauncher", "--my_sql.host=johnsql", "--my_sql.port=3306"]
